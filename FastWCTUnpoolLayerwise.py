...
parser.add_argument("--outf", default="unpool/", help='folder to output images and model checkpoints')
parser.add_argument("--content_layers", default="r41", help='layers for content')
parser.add_argument("--style_layers", default="r11", help='layers for style')
parser.add_argument("--batchSize", type=int,default=1, help='batch size')
parser.add_argument("--niter", type=int,default=50000, help='iterations to train the model')
...
#matrix.load_state_dict(torch.load('adapool/r31.pth'))
################# TRAINING #################
...
for iteration in range(1,opt.niter+1):
    ...
    # preprocess data
    try:
        content,cmask,_ = content_loader.next()
    except IOError:
        content,cmask,_ = content_loader.next()
    ...
    try:
        style,smask,_ = style_loader.next()
    except IOError:
        style,smask,_ = style_loader.next()
    except StopIteration:
        style_loader = iter(style_loader_)
        style,smask,_ = style_loader.next()
    # cmask: cx1xhxw, smask: cx1xhxw
    ...
    if(opt.mode == 'unpool'):
        feature,transmatrix = matrix(cF[opt.layer],sF[opt.layer],contentV,styleV)
        if(opt.layer == 'r11'):
        ...
        if(opt.layer == 'r41'):
            feature,transmatrix = matrix(cF[opt.layer],sF[opt.layer],contentV,styleV,cmask,smask)
        else:
            feature,transmatrix = matrix(cF,sF,contentV,styleV,cmask,smask)
        transfer = dec(feature)
    ...
    loss,styleLoss,contentLoss = criterion(tF,sF_loss,cF_loss,smask,cmask)
    ...
    if((iteration) % opt.log_interval == 0):
        ...
        cmask = cmask.view(1,1,256,256).float()
        content = content * cmask.repeat(1,3,1,1)

        smask = smask.view(1,1,256,256).float()
        style = style * smask.repeat(1,3,1,1)

        concat = torch.cat((content,style,transfer.data.cpu()),dim=0)
        vutils.save_image(concat,'%s/%d.png'%(opt.outf,iteration),nrow=opt.batchSize)
        #vutils.save_image(content,'%s/content_%d.png'%(opt.outf,iteration),normalize=True,scale_each=True,nrow=opt.batchSize)
        #vutils.save_image(style,'%s/style_%d.png'%(opt.outf,iteration),normalize=True,scale_each=True,nrow=opt.batchSize)
        #vutils.save_image(transfer.data.cpu(),'%s/transfer_%d.png'%(opt.outf,iteration),normalize=True,scale_each=True,nrow=opt.batchSize)
